...
def create(request: DangKyThi, db: Session = Depends(get_db)):
    return db_dangkythi.create(db, request)

@router.get("", response_class=HTMLResponse)
def read_root(request: Request, db: Session = Depends(get_db)):

    return templates.TemplateResponse("formDangKyThi.html", {"request": request})
@router.get("/monhocdk", response_model=List[MonHocDisplay])
def get_monhoc_dangky(magv: str, db: Session = Depends(get_db)):
