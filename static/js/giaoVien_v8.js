...
async function selectRow(index) {
  ...
  if (btnSua) { btnSua.disabled = false; btnSua.removeAttribute("data-disabled"); btnSua.style.opacity = 1; btnSua.style.cursor = "pointer"; btnSua.title = ""; }
  ...
  try {
    ...
    if (checkData.co_gan_mon || checkData.co_cauhoi || checkData.da_thi) {
      if (btnSua) { btnSua.disabled = false; btnSua.setAttribute("data-disabled", "true"); btnSua.style.opacity = 0.5; btnSua.style.cursor = "not-allowed"; }
...
if (maGVInput) {
    maGVInput.addEventListener('blur', formatIdOnBlur);
}
if (hoGVInput) {
