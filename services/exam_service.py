...
def list_class_exam_results(
    ...
    """Return all students in a class with their submitted exam result (session_id, score) for a kỳ thi."""
    from db.model import DbPhienThi, DbSinhVien
    ...
    if user and user.get("role") == "GIANGVIEN":
        ...
        if reg is None:
            raise PermissionDeniedError("Bạn không có quyền xem kết quả của kỳ thi này.")
    ...
    # Build a lookup: masv → submitted PhienThi
    submitted = (
        db.query(DbPhienThi)
        .filter(
    ...
    session_map: dict[str, DbPhienThi] = {}
    for s in submitted:
        masv_key = (s.masv or "").strip()
        # Keep the latest submission per student
        if masv_key not in session_map or (
            s.nopbai_luc and (
                session_map[masv_key].nopbai_luc is None
                or s.nopbai_luc > session_map[masv_key].nopbai_luc
            )
        ):
            session_map[masv_key] = s

    results = []
    for idx, sv in enumerate(students, start=1):
        masv = (sv.masv or "").strip()
        hoten = f"{(sv.ho or '').strip()} {(sv.ten or '').strip()}".strip()
        pt = session_map.get(masv)
        submitted_at = None
        if pt and (pt.nopbai_luc or pt.capnhat_luc):
            ts = pt.nopbai_luc or pt.capnhat_luc
            submitted_at = ts.strftime("%d/%m/%Y %H:%M")
        results.append({
            "stt": idx,
            "masv": masv,
            "hoten": hoten,
            "session_id": pt.id if pt else None,
            "diem": pt.diem if pt else None,
